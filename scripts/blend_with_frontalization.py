...
from utils.common import tensor2im, blend, makeupblend, latentblend, imagepostprocess
from utils.inference_utils import run_on_batch

MALE=True #Set True for male MBTI test
...
def run():
    ...
    if opts.frontal_name==None:
        if MALE:
            idx = np.random.randint(1,12) #Until now, we have frontal_face latent from 1 to 7.
            idx=42
            frontal_name='mfrontal_{}_01'.format(str(idx))
        else:
            idx = np.random.randint(1,8) #Until now, we have frontal_face latent from 1 to 7.
            idx=7
            frontal_name='frontal_{}_01'.format(str(idx))
    else:
        frontal_name=opts.frontal_name
    try:
        frontal_latent = torch.from_numpy(np.load('./latents/{}.npy'.format(frontal_name)))
        add_front=False
    except:
        image_list.append(frontal_name)
        add_front=True
    ...
    latent = blend(image_list[:-1] if add_front else image_list).to('cuda')
    frontal_latent = torch.from_numpy(np.load('./latents/{}.npy'.format(frontal_name))).to('cuda')
    new_latent=latentblend(frontal_latent,latent,'4,5').to('cuda')
    output, _ = net.decoder([new_latent], input_is_latent=True, randomize_noise=None, return_latents=False)
    output_2, _ = net.decoder([latent], input_is_latent=True, randomize_noise=None, return_latents=False)
    output = tensor2im(output[0])
    output_2=tensor2im(output_2[0])
    output.save('mix_new.png')
    output_2.save('mix_old.png')
    reference_image_path=os.path.join(opts.data_path,'{}.png'.format(frontal_name))
    if os.path.isfile('landmark/{}.png.npy'.format(frontal_name)):
        landmark=np.load('landmark/{}.png.npy'.format(frontal_name))
    else:
        landmark=None
    image_new=imagepostprocess(reference_image_path,'./mix_new.png',landmark,NOSE=True)
    cv2.imwrite('mix_gaussian.png', image_new)
